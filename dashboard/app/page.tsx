...
export default function HomePage() {
  ...
  const loadAnalytics = (hours: number) => {
    ...
    routerApi.orgAnalytics(hours).then(setOrgAnalytics).catch(() => {});
  };
  ...
  useEffect(() => {
    ...
    routerApi.orgAnalytics(720).then(setOrgAnalytics).catch(() => {});
    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6 bg-[#0F172A] border border-[#1E293B] shadow-xl">
          ...
          {/* Org-Wide Financial Rollup */}
          {orgAnalytics && (
            <div className="mt-6 pt-4 border-t border-[#1E293B] space-y-3 font-mono text-xs">
              <div className="flex justify-between items-center text-slate-400 font-bold text-[11px]">
                <span>Org-Wide Cross-Workspace Rollup</span>
                <span className="text-emerald-400 font-bold">${(orgAnalytics.total_spend || 0).toFixed(4)} total</span>
              </div>
              {orgAnalytics.workspace_breakdown && orgAnalytics.workspace_breakdown.length > 0 ? (
                <div className="space-y-1.5">
                  {orgAnalytics.workspace_breakdown.map((ws: any) => (
                    <div key={ws.workspace_id} className="flex justify-between items-center p-2 rounded-lg bg-[#090D16] border border-[#1E293B] text-[11px]">
                      <span className="text-slate-300">Workspace: <strong className="text-white">{ws.workspace_id}</strong></span>
                      <span className="text-slate-400">{ws.requests} reqs ({ws.tokens} tokens) — <strong className="text-emerald-400">${ws.spend.toFixed(4)}</strong></span>
                    </div>
                  ))}
                </div>
              ) : (
                <span className="text-slate-500 italic text-[11px] block">Single workspace active</span>
              )}
            </div>
          )}
        </Card>
