...
class MindClient:
    ...
    async def retrieve(
        self, text: str, workspace_id: str, top_k: int = 3, correlation_id: Optional[str] = None
    ) -> RetrievalOutcome:
        if not self.enabled or not self.client or not text.strip():
            return RetrievalOutcome(STATUS_UNAVAILABLE)
        hits = await self.query(text, workspace_id, top_k=top_k, correlation_id=correlation_id)
        if hits is None:
            return RetrievalOutcome(STATUS_UNAVAILABLE)
