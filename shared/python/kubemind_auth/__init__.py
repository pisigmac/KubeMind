...
def verify_opendesk_jwt(token: str) -> Dict[str, Any]:
...
class Authenticator:
    ...
    def authenticate(
        ...
        workspace_header: Optional[str],
        authorization_header: Optional[str] = None,
    ) -> AuthResult:
        # Check OpenDesk Bearer JWT token
        bearer_token = None
        if authorization_header and authorization_header.startswith("Bearer "):
            bearer_token = authorization_header[7:].strip()
        elif api_key and api_key.startswith("ey"):
            bearer_token = api_key

        if bearer_token and bearer_token.startswith("ey"):
            payload = verify_opendesk_jwt(bearer_token)
            ws = payload.get("workspace_id") or payload.get("org_id") or workspace_header or self.default_workspace
            if not valid_workspace(ws):
                raise AuthError("Invalid workspace identifier in JWT claims", status_code=400)

            roles = payload.get("roles", {})
            role_name = (
                roles.get("kubemind")
                if isinstance(roles, dict)
                else (payload.get("role") or Role.DEVELOPER.value)
            )
            scopes = ROLE_SCOPES.get(role_name, {"*"} if role_name == Role.ADMIN.value else set())

            return AuthResult(
                workspace_id=ws,
                authenticated=True,
                key_id=payload.get("sub", "opendesk-jwt"),
                role=role_name,
                scopes=scopes,
            )
